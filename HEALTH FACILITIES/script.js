...
L.geoJSON(healthFacilitiesData, {
    pointToLayer: function (feature, latlng) {
        ...
        const popupContent = `
            <div class="custom-popup">
                ...
                <div class="popup-image-wrap">
                    <img src="${feature.properties.image_url}" alt="${feature.properties.name}" onerror="this.onerror=null; this.src='https://placehold.co/240x120?text=No+Photo+Available';">
                </div>

                <p class="popup-desc">${feature.properties.desc}</p>
        ...
        geoMarker.bindPopup(popupContent, {
            maxWidth: 260,
            autoPanPadding: L.point(15, 15) // Keeps layout clear of edges when auto-focusing
        });
...
// Saved instance reference to manipulate styling during mobile drawer transitions
...
// 7. Interactive Sidebar Drawer Actions (Unified Screen Parity Solution)
...
function focusOnFacility(coords, markerInstance) {
    // If on mobile, pan map so marker drops cleanly into the top half of viewport
    if (window.innerWidth <= 768) {
        const targetZoom = 16;
        map.setView(coords, targetZoom);
        
        // Timeout prevents calculation stutter while mobile drawer bottom-sheet collapses
        setTimeout(() => {
            markerInstance.openPopup();
            // Offset positioning slightly down to prevent map text popups clipping underneath dashboards
            map.panBy([0, -80]); 
        }, 300);
